...
class NaiveBayes:
  ...
  def classify(self, words):
    ...
    # print (str(len(set(self.all_words))) +">>>>"+ str(len(self.word_dictionary.keys())))
    pos_prob=0
    neg_prob=0
    for word in words:
      if word in self.word_dictionary:
        pos_prob+=math.log((self.word_dictionary[word]['pos']+1)/(self.total_word_count['pos']+len(self.word_dictionary.keys())+1))
        neg_prob+=math.log((self.word_dictionary[word]['neg']+1)/(self.total_word_count['neg']+len(self.word_dictionary.keys())+1))
    pos_prob+=math.log(self.total_pos_doc/self.total_docs)
    neg_prob+=math.log(self.total_neg_doc/self.total_docs)
  ...
  def addExample(self, klass, words):
    ...
    self.total_docs+=1
    if klass=='pos':
      self.total_pos_doc+=1
    else:
      self.total_neg_doc+=1
    for word in words:
      self.total_words+=1
      # self.all_words.append(word)
      
      if word in self.word_dictionary:
        self.word_dictionary[word][klass]+=1
      else:
        self.word_dictionary[word]={}
        self.word_dictionary[word]['pos']=0
        self.word_dictionary[word]['neg']=0
        self.word_dictionary[word][klass]+=1
      
      
      self.total_word_count[klass]+=1
